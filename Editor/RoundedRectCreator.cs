...
namespace Editor.Custom
{
    public class RoundedRectCreator : EditorWindow
    {
        ...
        private void CreateRectImage(bool preview)
        {
            ...
            if (widthLine > 0)
            {
                saveImagePath = Path.Combine(saveDirectory, $"round_{radius}_line_{widthLine}");
            }
            else
            {
                saveImagePath = Path.Combine(saveDirectory, $"round_{radius}");
            }

            if (!leftBottomCorner || !leftTopCorner || !rightBottomCorner || !rightTopCorner)
            {
                if (leftBottomCorner)
                {
                    saveImagePath += "_left_bottom";
                }

                if (leftTopCorner)
                {
                    saveImagePath += "_left_top";
                }
                
                if (rightBottomCorner)
                {
                    saveImagePath += "_right_bottom";
                }
                
                if (rightTopCorner)
                {
                    saveImagePath += "_right_top";
                }
            }
        ...
        private void StartEmptyScene()
        {
            ...
            tempScene.name = "Temp scene_"+Random.Range(0,99999);
        }
